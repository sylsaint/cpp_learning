...
std::string Lz77::encode(std::string s, LzConfig cfg) {
    ...
    while(point <= s.length()) {
        ...
        auto li = r.length + 1;
        auto pi = r.pos;
        std::string ci1 = reprod::radix_alpha(alpha, pi - 1, prefix); 
        std::string ci2 = reprod::radix_alpha(alpha, li - 1, lz_config.ls);
        std::string ci3(1, *(buffer.begin()+prefix+li-1));
        std::cout << "ci3==" << *(buffer.begin()+prefix+li-1) << std::endl;
        std::cout << "buffer size: " << buffer.size() << std::endl;
        ...
        for(size_t i = 0; i < li; i++) {
            if(point >= s.length()) {
                // pading zeros
                buffer.push_back(lz_config.zero);
            } else {
